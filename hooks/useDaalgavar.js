...
function useDaalgavar(token, baiguullagiinId, query, order) {
  ...
  useEffect(() => {
    if (baiguullaga?._id) {
      socket().on(`baiguullaga${baiguullaga?._id}`, sonorduulga => {
        if (sonorduulga.turul === 'daalgavar') mutate();
      });
    }
    return () => {
  ...
  const jagsaalt = useMemo(() => {
    return [...(khuudaslalt?.jagsaalt || []), ...(data?.jagsaalt || [])].map(
      (v, i) => {
        v.dugaar = i + 1;
        return v;
      },
