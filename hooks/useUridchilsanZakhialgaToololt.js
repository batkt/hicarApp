...
const fetcher = (url, token, baiguullagiinId, ekhlekhOgnoo, duusakhOgnoo, ajiltniiId, salbariinId) => {
...
function useUridchilsanZakhialgaToololt(token, baiguullagiinId, zakhialgaMutate, ekhlekhOgnoo, duusakhOgnoo, ajiltniiId) {
    ...
    const { data, mutate } = useSWR(!!token && !!baiguullagiinId  && !!salbariinId ? ['/zakhialgiinTooAvya', token, baiguullagiinId, ekhlekhOgnoo, duusakhOgnoo, ajiltniiId, salbariinId] : null, fetcher, { revalidateOnFocus: false })

    useEffect(() => {
        if (!!baiguullagiinId) {
            socket().on(`baiguullaga${baiguullagiinId}`, () => {
                mutate()
                zakhialgaMutate()
            });
        }
