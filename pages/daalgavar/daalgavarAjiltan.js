...
export default function DaalgavarAjiltan({title, ...props}) {
  ...
  const {jagsaalt, daalgavarMutate, refresh, daalgavarGaralt, next} =
      useDaalgavar(token, token, query, order);
...
/*
  function daalgavarIlgeeh() {
    const daalgavarUrl = daalgavarHuleejAvah?.tuluv === 0 ? '/daalgavarKhuleejAvlaa' : '/daalgavarDuusgalaa';
    uilchilgee(token)
        .post(daalgavarUrl, {id: daalgavarHuleejAvah?._id})
        .then(({data}) => {
          if (data === 'Amjilttai') {
            refresh();
            onClose();
  ...
  return (
      <Box bg="#1877f2" style={{flex: 1}}>
        ...
        {/*<AlertDialog
            ...
            isOpen={!!daalgavarHuleejAvah}
            onClose={onClose}>
          <AlertDialog.Content>
            <AlertDialog.CloseButton />
            <AlertDialog.Header>Даалагвар</AlertDialog.Header>
            <AlertDialog.Body>
              {daalgavarHuleejAvah?.tuluv===0 ? 'Та даалгавар хүлээж авах уу?' : 'Та даалгавараа дуусгах уу?'}
            </AlertDialog.Body>
            <AlertDialog.Footer>
