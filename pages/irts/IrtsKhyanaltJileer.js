...
const IrtsKhyanaltJileer = props => {
  const {sonorduulga, ajiltan, token, salbariinId} = useAuth();
  const [chosenYear, setChosenYear] = useState(moment().format('YYYY'));
  const [ognoo, setOgnoo] = useState([moment().toDate(), moment().toDate()]);
  const navigation = useNavigation();

  const queryData = useMemo(() => {
    return {
      ekhlekhOgnoo: moment(ognoo[0]).startOf('year').format('YYYY-MM-DD 00:00:00'),
      duusakhOgnoo: moment(ognoo[1]).endOf('year').format('YYYY-MM-DD 23:59:59'),
      salbariinId,
    }
  ...
  function DateUnits() {
    const sar = new Array(10).fill('').map((item,index)=>{
      const amount = moment().year() - 2022;
      return moment().subtract(amount, 'year').add(index,'year');
      // return moment().add(index,'year');
    });

    return (
        <>
          {sar.map((mur, index)=>
              <Pressable w="20" onPress={() => {setChosenYear(mur.format('YYYY')); setOgnoo([mur,mur])}}>
                <Center
                    bg={chosenYear === mur.format('YYYY') ? 'blue.600' : 'white'}
                    rounded={'20px'} h="55" w="20">
                  <Text color={chosenYear === mur.format('YYYY') ? 'white' : 'gray.400'} fontSize={14} fontWeight="bold">
                    {/*{moment().year()}*/}
                    {mur.format('YYYY')}
                  </Text>
                </Center>
...
export default IrtsKhyanaltJileer;
